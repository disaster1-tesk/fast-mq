...
public class FastMQServiceImpl implements FastMQService {
    ...
    public void consumeIdleMessages(Set<StreamMessageId> idleIds, FastMQListener fastMQListener) {
        ...
        Map<StreamMessageId, Map<Object, Object>> messages = stream.readGroup(Objects.isNull(fastMQMessageListener) ? FastMQConstant.DEFAULT_CONSUMERGROUP : fastMQMessageListener.groupName(),
    ...
    public void consumeDeadLetterMessages(Set<StreamMessageId> deadLetterIds, FastMQListener fastMQListener) {
        ...
        for (StreamMessageId id :
            ...
            if (!CollectionUtils.isEmpty(range)) {
                ...
                stream.remove(id);
                long ack = stream.ack(Objects.isNull(fastMQMessageListener) ? FastMQConstant.DEFAULT_CONSUMERGROUP
    ...
    public void claimIdleConsumer(FastMQListener fastMQListener) {
        ...
        RFuture<PendingResult> infoAsync = stream.getPendingInfoAsync(Objects.isNull(fastMQMessageListener) ? FastMQConstant.DEFAULT_CONSUMERGROUP
                : fastMQMessageListener.groupName());
        PendingResult res = stream.getPendingInfo(Objects.isNull(fastMQMessageListener) ? FastMQConstant.DEFAULT_CONSUMERGROUP
                : fastMQMessageListener.groupName());
        //获取组内的所有消费者的名称
        ...
        for (PendingEntry entry :
            ...
            //转移消息
            stream.claim(Objects.isNull(fastMQMessageListener) ? FastMQConstant.DEFAULT_CONSUMERGROUP
    ...
    public void consumeFastMQListeners(FastMQListener fastMQListener) {
        ...
        Map<StreamMessageId, Map<Object, Object>> res = stream.readGroup(Objects.isNull(fastMQMessageListener) ? FastMQConstant.DEFAULT_CONSUMERGROUP
                        : fastMQMessageListener.groupName(),
                Objects.isNull(fastMQMessageListener) ? FastMQConstant.DEFAULT_CONSUMER
                        : fastMQMessageListener.consumeName(),
        ...
        if (Objects.nonNull(fastMQMessageListener) && fastMQMessageListener.idempotent()) {
            ThreadPoolUtil.run(() -> {
                ...
                if (Objects.isNull(fastMQMessageListener)) {
                    //ACK机制，比pubsub优秀
                    long ack = stream.ack(FastMQConstant.DEFAULT_CONSUMERGROUP, id);
                    printAckLog(id, fastMQMessageListener, ack);

                } else {
                    long ack = stream.ack(fastMQMessageListener.groupName(), id);
                    printAckLog(id, fastMQMessageListener, ack);
                }
            });
        } else {
